...
data class OrgRules constructor(
  ...
  fun getKeyManagerUrlForPublicKeys(): String? {
    if (flags?.firstOrNull { it == DomainRule.NO_KEY_MANAGER_PUB_LOOKUP } != null) {
      return null
    }
    return keyManagerUrl
  }
  ...
  fun canBackupKeys(): Boolean {
    return flags?.firstOrNull { it == DomainRule.NO_PRV_BACKUP } == null
  }
  ...
  fun mustSubmitToAttester(): Boolean {
    return flags?.firstOrNull { it == DomainRule.ENFORCE_ATTESTER_SUBMIT } != null
  }
  ...
  fun rememberPassPhraseByDefault(): Boolean {
    return flags?.firstOrNull { it == DomainRule.DEFAULT_REMEMBER_PASS_PHRASE } != null
  ...
  fun mustAutoGenPassPhraseQuietly(): Boolean {
    return flags?.firstOrNull { it == DomainRule.PASS_PHRASE_QUIET_AUTOGEN } != null
  }
  ...
  fun canSubmitPubToAttester(): Boolean {
    return flags?.firstOrNull { it == DomainRule.NO_ATTESTER_SUBMIT } == null
  }
  ...
  fun useLegacyAttesterSubmit(): Boolean {
    return flags?.firstOrNull { it == DomainRule.USE_LEGACY_ATTESTER_SUBMIT } != null
  }
  ...
  fun shouldHideArmorMeta(): Boolean {
    return flags?.firstOrNull { it == DomainRule.HIDE_ARMOR_META } != null
  }
