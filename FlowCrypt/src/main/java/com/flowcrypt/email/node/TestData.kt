...
class TestData internal constructor() {
  companion object {
    ...
    @JvmStatic
    fun payload(mb: Int): ByteArray {
      val data = ByteArray(if (mb == 0) 100 else mb * 1024 * 1024) // min 100 bytes
      Arrays.fill(data, 'X'.toByte())
      return data
    }
    ...
    fun eccPrvKeyInfo(): Array<KeyEntity> {
      return arrayOf(KeyEntity(
          ...
          passphraseType = KeyEntity.PassphraseType.REGULAR
      ))
    }
