...
data class OrgRules constructor(
  ...
  fun getKeyManagerUrlForPublicKeys(): String? {
    if (flags?.first { it == DomainRule.NO_KEY_MANAGER_PUB_LOOKUP } != null) {
      return null
    }
    return keyManagerUrl
  }
  ...
  fun canCreateKeys(): Boolean {
    return flags?.first { it == DomainRule.NO_PRV_CREATE } == null
  }
  ...
  fun canBackupKeys(): Boolean {
    return flags?.first { it == DomainRule.NO_PRV_BACKUP } == null
  }
  ...
  fun mustSubmitToAttester(): Boolean {
    return flags?.first { it == DomainRule.ENFORCE_ATTESTER_SUBMIT } != null
  }
  ...
  fun rememberPassPhraseByDefault(): Boolean {
    return flags?.first { it == DomainRule.DEFAULT_REMEMBER_PASS_PHRASE } != null
  ...
  fun mustAutoImportOrAutoGenPrvWithKeyManager(): Boolean {
    if (flags?.first { it == DomainRule.PRV_AUTOIMPORT_OR_AUTOGEN } == null) {
      return false
    }
  ...
  fun mustAutoGenPassPhraseQuietly(): Boolean {
    return flags?.first { it == DomainRule.PASS_PHRASE_QUIET_AUTOGEN } != null
  }
  ...
  fun canSubmitPubToAttester(): Boolean {
    return flags?.first { it == DomainRule.NO_ATTESTER_SUBMIT } == null
  }
  ...
  fun useLegacyAttesterSubmit(): Boolean {
    return flags?.first { it == DomainRule.USE_LEGACY_ATTESTER_SUBMIT } != null
  }
  ...
  fun shouldHideArmorMeta(): Boolean {
    return flags?.first { it == DomainRule.HIDE_ARMOR_META } != null
  }
